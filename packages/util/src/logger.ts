...
export function fmt16(v: number, bare = false) {
  return `${!bare ? '0x' : ''}${(v & 0xffff).toString(16).toUpperCase().padStart(4, '0')}`
}

export function fmt8(v: number, bare = false) {
  return `${!bare ? '0x' : ''}${(v & 0x00ff).toString(16).toUpperCase().padStart(2, '0')}`
}
